...
// Daten holen Anfang
 Partie = window.setInterval(makeMove, 1000);
  function makeMove() {setRequest_restore();
...
//if(ini == 0){user_nachricht ='Kein Spieler am Brett'; init();}
}
...
if (Check == true)  {user_nachricht =w_player+"::untenschieden::"+b_player+"::Zeit"; init(); return;}
...
//var remis_mitt = nachricht_daten[1];
 ...
 if (w_player != "0" && b_player == "0" ){user_nachricht = w_player+" hat die Partie betreten"; sound(); w_zeit =0; b_zeit =0; }
...
if (w_player != "0" && b_player != "0" && diff_time >= del_partie_zeit){user_nachricht ='Partie ist abgelaufen'; init(); return;}
  ...
  if (w_player == "0" && b_player == "0" && diff_time > 3){
...
if (wer_zieht=="w" && w_zeit_player >= partie_zeit){user_nachricht =b_player+"::gewinnt::"+w_player+"::Zeit"; init(); return;}
if (wer_zieht=="b" && b_zeit_player >= partie_zeit){user_nachricht =w_player+"::gewinnt::"+b_player+"::Zeit"; init(); return;}
if (wer_zieht=="w" && game.in_checkmate() === true){user_nachricht =b_player+"::gewinnt::"+w_player+"::Matt"; init(); return;}
if (wer_zieht=="b" && game.in_checkmate() === true){user_nachricht =w_player+"::gewinnt::"+b_player+"::Matt"; init(); return;}
...
if (game.turn() === 'w' && weiss == 1) {weiss++; sound();ini = 0;}
 ...
 if ( b_player == "0" || wer_zieht=="b") {<?php echo $botty ?>}
}
//---------------------------------------------------------------------
// Daten holen Ende 
...
//if (w_player == "0" ||  b_player == "0" ){user_nachricht ="Keine Aufgabe moeglich"; return;}user_nachricht =w_player+"::gewinnt::"+b_player+"::Aufgabe"; 
if (w_player == user ){user_nachricht =b_player+"::gewinnt::"+w_player+"::Aufgabe"; init(); return;}
if (b_player == user ){user_nachricht =w_player+"::gewinnt::"+b_player+"::Aufgabe"; init(); return;}
});
//---------------------------------------------------------------------
...
var ts = Math.round((new Date()).getTime() / 1000);
  ...
  var partie_daten =  '1##0##0##0##'+ts+'##0##0##w##rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1##'+nachricht+'##'+user_nachricht;
...
<div id="box1">
  <table width="100%"  border="0">
    <tr>
      <td colspan="2" align="left"> <div align="left"><span class="Stil3"><span id="w_player">0</span > - <span id="w_zeit_player">000</span> -:- <span id="b_zeit_player">000</span> - <span id="b_player">0</span></span>
      </td>
    </tr>
    <tr>
      <td colspan="2">Board: <span id="nachricht"></span></td>
    </tr>
    <tr>
      <td width="26%" colsp
an="3"></td>    </tr>
  ...
  <input name="send_button" type="submit"  value="Partie betreten" style="width: 150px; height: 25px; font-size:15px; color:#666666;"><input name="player" type="hidden" id="player" value="<?php echo $user; ?>">
      ...
      <td width="74%"><form name="MyForm4" action="" method="post" onsubmit="xmlhttpPost('reklamieren.php', 'MyForm4', 'nachricht', '<img src=\'pleasewait.gif\'>'); return false;">
          <input name="send_button" type="submit"  value="Gewinn reklamieren" style="width: 150px; height: 25px; font-size:15px; color:#666666;">
          <input name="user_nachricht" type="hidden" value="Gewinn reklamieren"   >
          <input name="player" type="hidden" id="player" value="<?php echo $user; ?>">
      </form></td>
    </tr>
    <tr>
      <td colspan="2"><input type="button" style="width: 150px; height: 25px; font-size:15px; color:#666666;" id="aufgabe" value="Partie aufgeben" /></td>
    </tr>
    <tr>
      <td colspan="2"><form name="MyForm3" action="" method="post" onsubmit="xmlhttpPost('nachricht.php', 'MyForm3', 'nachricht', '<img src=\'pleasewait.gif\'>'); return false;">
    ...
    <tr>
      <td colspan="2"><span class="Stil2">Nachricht: <span id="user_nachricht"></span> </span></td>
    </tr>
    <tr>
      <td colspan="2"><form name="MyForm2" action="" method="post" onsubmit="xmlhttpPost('nachricht.php', 'MyForm2', 'nachricht', '<img src=\'pleasewait.gif\'>'); return false;">
         <p align="left">
          <input name="player" type="hidden" id="player" value="<?php echo $user; ?>">
